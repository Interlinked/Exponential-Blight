...
namespace ExponentialBlight
{
    ...
    {   
        ...
        public void Awake()
        {
            ...
            myItemDef.name = "EXPONENTIAL_BLIGHT";
            myItemDef.nameToken = "EXPONENTIAL_BLIGHT";
            myItemDef.pickupToken = "Deal more damage the more stacks of blight you have on an enemy.";
            myItemDef.descriptionToken = "Deal more damage the more stacks of blight you have on an enemy.";
            myItemDef.loreToken = "EXPONENTIAL_BLIGHT_LORE";
...
#pragma warning restore Publicizer001
        ...
        {   
            var dotStackList = self.dotStackList;
            var  expBlightCount = 0;
            if (attackerObject != null && attackerObject.GetComponent<CharacterBody>().inventory != null)
            {
                expBlightCount = attackerObject.GetComponent<CharacterBody>().inventory.GetItemCount(myItemDef.itemIndex);
            }
           
            if (dotIndex == DotController.DotIndex.Blight)
            {
                var blightStackCount = 0;
                for (int i = 0; i < dotStackList.Count; i++) {
                    if (dotStackList[i].dotIndex == DotController.DotIndex.Blight) {
                        blightStackCount++;
                    }
                }
                damageMultiplier = (float)(damageMultiplier * Math.Pow((1.6*expBlightCount), blightStackCount));
            }
        ...
        // Handler for the onCharacterDeathGlobal event
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.F2))
            {
                ...
                ModLogger.LogInfo($"Player pressed F2. Spawning our custom item at coordinates {transform.position}");
